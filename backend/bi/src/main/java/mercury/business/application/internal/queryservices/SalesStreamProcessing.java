...
public class SalesStreamProcessing {
    ...
    @Bean
    public Function<KStream<?, Backlog>, KStream<String, Double>> processBacklog() {
        df.setRoundingMode(RoundingMode.DOWN);
        return backlogStream -> {
            KTable<String, Double> backlogKTable = backlogStream.map((k, v) -> {
            ...
            KStream<String, Double> saleIntelStream = backlogKTable.toStream()
                    .map((k, v) -> KeyValue.pair(k, Double.valueOf(df.format(v))));
            saleIntelStream.print(Printed.<String, Double>toSysOut().withLabel("Console Output"));

            return saleIntelStream;
        };
    }
}
